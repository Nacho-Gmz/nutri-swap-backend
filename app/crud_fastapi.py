    ...
    if conn:
        try:
            ...
            if alimentos:
                ...
                raise HTTPException(status_code=404, detail="No se encontraron alimentos dentro de la misma categoría")
        except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))
        finally:
            conn.close()
...
async def crear_usuario(usuario: UsuarioCreate):
    ...
    if conn:
        try:
            ...
            """
            cursor.execute(query, (usuario.nombre, usuario.apellidos, usuario.correo, hashed_contrasenia))
            conn.commit()
            new_id = cursor.fetchone()[0]
...
async def obtener_contra_usuario(usuario_id: int):
    ...
    if conn:
        try:
            cursor = conn.cursor(cursor_factory=RealDictCursor)
            cursor.execute("SELECT contrasenia FROM usuarios WHERE id = %s;", (usuario_id,))
...
async def obtener_usuario(usuario_id: int):
    ...
    if conn:
        try:
            cursor = conn.cursor(cursor_factory=RealDictCursor)
            cursor.execute("SELECT id, nombre, apellidos, correo FROM usuarios WHERE id = %s;", (usuario_id,))
...
async def actualizar_usuario(usuario_id: int, usuario: UsuarioCreate):
    ...
    if conn:
        try:
            ...
            """
            cursor.execute(query, (usuario.nombre, usuario.apellidos, usuario.correo, hashed_contrasenia, usuario_id))
            conn.commit()
            if cursor.rowcount > 0:
